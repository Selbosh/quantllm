...
def get_lat_long(city_name: str):
  ...
  if location is None:
    raise ValueError(f"{city_name} not found on <https://nominatim.openstreetmap.org>")
...
class WeatherAPI:
  ...
  def get_weather_range(self, lat, lon,
    ...
    """
    start_dates = [f'{year}-{month}-01' for year in range(start_year, end_year + 1)]
    end_dates = [f'{year}-{month}-31' for year in range(start_year, end_year + 1)]
    responses = [self.get_weather(lat, lon, start, end)[0] for (start, end) in zip(start_dates, end_dates)]
    return responses
  ...
  def get_cities_weather(self):
    ...
    for city in self.cities:
      ...
      meteo_responses = self.get_weather_range(loc['latitude'], loc['longitude'])
